...
function VerifyEmailInner() {
  ...
  return <VerifyEmailLayout status={status} message={message} />
}

function VerifyEmailLayout({ status, message }: { status: Status; message: string }) {
  return (
    <main className="min-h-screen bg-[#050915] text-white flex items-center justify-center px-4">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#0b132c] p-8 shadow-2xl text-center space-y-4">
        ...
        <div className="pt-2">
          ...
          {status === "success" && (
            <Link href="/" className="radius-6 btn btn-primary mt-2 inline-flex justify-center">
              Go to sign in
            </Link>
          )}
          {(status === "error" || status === "missing") && (
            <Link href="/" className="radius-6 btn mt-2 inline-flex justify-center">
              Back to sign in
            </Link>
          )}
        </div>
...
function VerifyEmailSkeleton() {
  return <VerifyEmailLayout status="checking" message="Preparing verification UI…" />
}
